...
def prepare_output_dirs(prefix='.output/'):
    ...
    if os.path.exists(pose_dir) and os.path.isdir(pose_dir):
        print("no deleting existing content")
...
def parse_args():
    ...
    parser.add_argument('--cfg',
                        ...
                        default='experiments/coco/hrnet/mp2-w32_384x288_adam_lr1e-3.yaml',
                        type=str)

    parser.add_argument('--outputDir', type=str, default='output/')
    parser.add_argument('--writeBoxFrames', default='True', action='store_true')
...
def main():
    ...
    #     print('desired inference fps is ' +
    #           str(args.inferenceFps)+' but video fps is '+str(fps))
    #     exit()
    # skip_frame_cnt = round(fps / args.inferenceFps)

    # path = "/home/kickbox/Downloads/"
    prefix=""
    prefix="sample_"
    path = "/home/kickbox/Documents/PROJECTS-linux/mp2/back_office/application/sample/"
    # path = "/home/kickbox/Documents/PROJECTS-linux/mp2/experiments/datasets/custom_images/mp2/random_1000/"
    filenames = glob.glob(
        path+"*.*")
        # path+"*.jp*")
    # filenames = glob.glob(args.img_file)
    print(filenames)
    count = 0
    for i, filename in enumerate(filenames):
        ...
        img_file = os.path.join(pose_dir, prefix+str(count)+".jpg")
        new_csv_row = [img_file]

        for coords in pose_preds:
            # Draw each point on image
            # for coord in coords:
            for idx, coord in enumerate(coords):
                if idx in NEEDED_POINTS:
                    # print(idx)
        ...
        #     print(1)
            # put something here when needed
            # break
        
        csv_output_rows.append(new_csv_row)
        
        print("outfile ", img_file)
        cv2.imwrite(img_file, cv2.cvtColor(image_debug, cv2.COLOR_RGB2BGR))
